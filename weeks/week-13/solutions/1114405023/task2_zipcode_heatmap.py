...
ZIPCODE_TO_COUNTY = {
    ...
    "600": "嘉義市",
    "602": "嘉義縣", "603": "嘉義縣", "604": "嘉義縣", "605": "嘉義縣",
...
def load_county_counts(year: int, data_dir: Path = DATA_DIR) -> dict[str, int]:
    ...
    with csv_path.open("r", encoding="utf-8-sig", newline="") as f:
        ...
        if not reader.fieldnames or "郵遞區號" not in reader.fieldnames:
            raise ValueError(f"{csv_path} 缺少「郵遞區號」欄位")

        for row in reader:
            zipcode = (row.get("郵遞區號") or "").strip()
...
def get_top_counties(all_years: dict[int, dict[str, int]], top_n: int = 10) -> list[str]:
    ...
    for year_data in all_years.values():
        total_counter.update(year_data)

    return [county for county, _count in total_counter.most_common(top_n)]
...
def plot_heatmap(
    ...
    fig_height = max(6, len(counties) * 0.5)
    ...
    ax.set_xlabel("學年度")
    ax.set_ylabel("縣市")
    ax.set_title("109 至 114 學年度來源縣市招生人數熱力圖")
...
def main() -> None:
    ...
    print(f"Task 2 圖表已輸出：{output_path}")
